  ...
  export default {
    ...
    methods: {
      ...
      async loadDraft(draftId) {
        const draft = this.drafts.find(d => d._id === draftId);
        if (draft) {
          this.form = { ...draft, date: '', month: '', year: '', contactNumber: '', email: this.user.email, signature: this.user.name };
      ...
      async saveDraft() {
        try {
          const draft = { ...this.form, _id: String(this.drafts.length + 1), createdAt: new Date() };
      ...
      resetForm() {
        this.form = {
          ...
          signature: this.user ? this.user.name : ''
        };
      },
      formatDate(date) {
